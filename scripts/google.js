...
module.exports = (robot) => {
  ...
  robot.hear(/!streamyard\b/, (res) => {
    ...
    if (!isAllowedStreamYardUser(room)) {
      return robot.messageRoom(
        room,
        "Sorry, you don't have permission to use StreamYard. 🚫"
      )
    }
  ...
  robot.hear(/!youtube\b/, (res) => {
    youtube
      ...
      .then((streams) =>
        streams.map((stream) => {
          const date = DateTime.fromISO(stream.snippet.scheduledStartTime, {
            locale: 'pt-br',
        ...
        streams.map((stream) => {
          ...
          return [
            ...
            stream.snippet.description
              ? preventUrlExpansion(stream.snippet.description)
                .replace(/\r/gm, '')
                .replace(/^/gm, '> ')
              : null
          ]
